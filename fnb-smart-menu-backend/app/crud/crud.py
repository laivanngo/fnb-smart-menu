...
async def update_product(db: AsyncSession, product_id: int, product: schemas.ProductUpdate):
    ...
    # Reload lại để trả về đủ info cho schema
...
async def link_product_to_options(db: AsyncSession, product_id: int, option_ids: List[int]):
    stmt = select(models.Product).options(selectinload(models.Product.options)).where(models.Product.id == product_id)
    result = await db.execute(stmt)
    db_prod = result.scalars().first()
    if not db_prod: return None
...
async def get_options(db: AsyncSession, skip: int = 0, limit: int = 100):
    stmt = select(models.Option).options(selectinload(models.Option.values)).order_by(models.Option.display_order.asc()).offset(skip).limit(limit)
    result = await db.execute(stmt)
    return result.scalars().all()
...
async def update_option(db: AsyncSession, option_id: int, option_update: schemas.OptionUpdate):
    ...
    # Reload values
    stmt_reload = select(models.Option).options(selectinload(models.Option.values)).where(models.Option.id == option_id)
...
async def get_option_values_by_ids(db: AsyncSession, value_ids: List[int]):
    stmt = select(models.OptionValue).where(models.OptionValue.id.in_(value_ids))
    result = await db.execute(stmt)
    return result.scalars().all()

# ==========================================
# 5. PUBLIC MENU (ĐÃ THÊM LẠI HÀM NÀY)
# ==========================================
...
async def update_user_points(db: AsyncSession, user_id: int, points_change: int, money_spent: float = 0):
...
async def get_voucher_by_code(db: AsyncSession, code: str):
    stmt = select(models.Voucher).where(models.Voucher.code == code, models.Voucher.is_active == True)
    result = await db.execute(stmt)
    return result.scalars().first()

# ==========================================
# 8. ORDER (CLEAN VERSION)
# ==========================================
async def create_order_record(db: AsyncSession, order_data: schemas.OrderCreate, calculated_data: dict, user_id: Optional[int]):
    db_order = models.Order(
        customer_name=order_data.customer_name,
        customer_phone=order_data.customer_phone,
        customer_address=order_data.customer_address,
        customer_note=order_data.customer_note,
        sub_total=calculated_data['sub_total'],
        delivery_fee=calculated_data['delivery_fee'],
        discount_amount=calculated_data['discount_amount'],
        points_discount=calculated_data['points_discount'],
        total_amount=calculated_data['total_amount'],
        delivery_method_selected=order_data.delivery_method,
...
async def create_order_items(db: AsyncSession, order_id: int, items_data: List[dict]):
    for item in items_data:
        ...
        if 'options_selected' in item:
            for opt in item['options_selected']:
                db_opt = models.OrderItemOption(
                    order_item_id=db_item.id,
                    option_name=opt.get('option_name', 'Option'),
                    value_name=opt['value_name'],
                    added_price=opt['added_price']
                )
                db.add(db_opt)
    await db.commit()
